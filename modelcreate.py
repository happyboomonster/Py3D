...
importq = input("Do you want to import a model, or start a new one? Any answer other than 'NEW' will be taken as a filename for an Object: ")
if(importq == "NEW"): #continue with an empty model[] list
    pass
else:
    try:
        infile = open(importq,"rb+")
...
while not Exit:
    ...
    while True:
        color = input("Give me a color out of these options or type EXIT to finish, REMOVE/EDIT to manipulate a triangle: " + str(colors) + " : ")
        if(color == "EXIT"): #do we want to leave?
            #get a filename and dump stuff there...
            filename = input("Filename? ")
            mifile = open(filename,"wb+")
            pickle.dump(model,mifile,protocol=2)
            mifile.flush()
            ...
            for x in range(0,len(model)):
                print(str(model[x]) + ",")
            removal = input("Which index do you want to remove? ")
            ...
            for x in range(0,len(model)):
                print(str(model[x]) + ",")
            edit = input("Which index do you want to edit? ")
            print("Here's the triangle you picked: " + str(model[int(edit)]))
            edittype = input("What modification do you want to make? Remake 'points', or change 'color'? ")
            if(edittype == 'points'): #now we break from the loop to get coordinates using the below method, but instead insert them back into the triangle
                ...
                newcolor = input("Please specify the new color: " + str(colors) + " ")
    ...
    while not Exit and not remove:
        triangleA = input("Please give triangle coords as positions from 0-255. (coordX-" + str(len(triangle)) + "): ")
        triangleB = input("Please give triangle coords as positions from 0-255. (coordY-" + str(len(triangle)) + "): ")
        triangleC = input("Please give triangle coords as positions from 0-255. (coordZ-" + str(len(triangle)) + "): ")
        try:
